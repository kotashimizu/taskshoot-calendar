-- RLS Performance Optimization Migration (FIXED VERSION)
...
-- CORRECTED: profiles table uses 'id' column, not 'user_id'

-- Migration: optimize_rls_performance_fixed
-- Created: 2025-01-19
...
-- Create optimized policies (CORRECTED: using 'id' instead of 'user_id')
...
-- ============================================
-- CATEGORIES TABLE POLICIES (Uses 'user_id')
-- ============================================
...
-- ============================================
-- TASKS TABLE POLICIES (Uses 'user_id')
-- ============================================
...
-- ============================================
-- GOOGLE CALENDAR CONFIGS TABLE POLICIES (Uses 'user_id')
-- ============================================
...
-- ============================================
-- SYNC LOGS TABLE POLICIES (Uses 'user_id')
-- ============================================
...
-- ============================================
-- GOOGLE EVENT SYNC TABLE POLICIES (Uses 'user_id')
-- ============================================
...
-- ============================================
-- TIME ESTIMATES TABLE POLICIES (Uses 'user_id')
-- ============================================
...
-- ============================================
-- TIME RECORDS TABLE POLICIES (Uses 'user_id')
-- ============================================
...
-- ============================================
-- TIME ESTIMATE HISTORY TABLE POLICIES (Uses 'user_id')
-- ============================================
...
COMMENT ON TABLE public.profiles IS 'User profiles - RLS optimized for performance (uses id column)';
COMMENT ON TABLE public.categories IS 'Task categories - RLS optimized for performance (uses user_id column)';
COMMENT ON TABLE public.tasks IS 'User tasks - RLS optimized for performance (uses user_id column)';
COMMENT ON TABLE public.google_calendar_configs IS 'Google Calendar configurations - RLS optimized for performance (uses user_id column)';
COMMENT ON TABLE public.sync_logs IS 'Synchronization logs - RLS optimized for performance (uses user_id column)';
COMMENT ON TABLE public.google_event_sync IS 'Google event synchronization - RLS optimized for performance (uses user_id column)';
COMMENT ON TABLE public.time_estimates IS 'Time estimates - RLS optimized for performance (uses user_id column)';
COMMENT ON TABLE public.time_records IS 'Time records - RLS optimized for performance (uses user_id column)';
COMMENT ON TABLE public.time_estimate_history IS 'Time estimate history - RLS optimized for performance (uses user_id column)';

-- ============================================
-- MIGRATION SUMMARY (CORRECTED)
-- ============================================

/*
OPTIMIZATION SUMMARY (CORRECTED VERSION):
- Total policies optimized: 24
- Tables affected: 9 tables with correct column mapping:
  * profiles: uses 'id' column for user identification
  * All other tables: use 'user_id' column for user identification
...
- Other tables:   (SELECT auth.uid()) = user_id
